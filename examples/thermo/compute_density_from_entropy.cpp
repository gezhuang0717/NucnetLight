...
int main( int argc, char * argv[] ) {
  ...
  if( argc < 5 )
  {
    fprintf(
      stderr,
      "Usage: argv[0] input_net_xml t9 s/b Ye [xpath constraint]\n\n"
    );
    fprintf(
      stderr,
    ...
    fprintf(
      stderr,
      "  t9 = input temperature in 10^9 K\n\n"
    );
    fprintf(
      stderr,
  ...
  Libnucnet__Zone__updateProperty( p_zone, nnt::s_T9, NULL, NULL, argv[2] );
  ...
  std::cout << std::endl <<
    ...
    user::compute_thermo_quantity(
      wrapped_zone, "entropy per nucleon", "electron"
    ) <<
    std::endl;
  ...
  std::cout << std::endl <<
    ...
    user::compute_thermo_quantity(
      wrapped_zone, "entropy per nucleon", "photon"
    ) <<
    std::endl;
...
void
print_equilibrium( nnt::Zone& zone )
{

  Libnuceq *p_equil;
  
  p_equil = nnt::get_zone_equilibrium( zone );
