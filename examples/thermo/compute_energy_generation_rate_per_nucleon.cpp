...
//!
//! \file
//! \brief Example code to compute the energy generation rate per
...
#define S_EG_RATE "energy generation rate"
...
check_input( int argc, char **argv )
{
  ...
  if( argc != 3 )
  {
    ...
    std::cout << "Purpose: " << argv[0] <<
      " computes energy generation rate (MeV per nucleon) from a network output xml file" <<
      std::endl;
    fprintf(
...
int main( int argc, char * argv[] ) {
  ...
  BOOST_FOREACH( nnt::Zone zone, zones )
  {
    std::cout <<
      ...
      zone.getProperty( S_EG_RATE ) << std::endl;
  }
