...
)
{
  ...
  p_extra_data->pNseCorrFactors = NULL;
  ...
  if( self->pfNseCorrectionFactorFunction ) {
    ...
    xmlHashScan(
      p_view->pNet->pNuc->pSpeciesHash,
      (xmlHashScanner) Libnucnet__Zone__assignNseCorrectionFactors,
      p_extra_data
    );
    ...
    gsl_vector_free( p_extra_data->pNseCorrFactors );
...
Libnucnet__Net__computeScreeningFactorForReaction(
  ...
  double d_rho,
  double d_ye,
  Libnucnet__Net__screening_function pf_screening_function,
  void *p_user_data
)
{
  ...
  typedef struct {
    ...
    unsigned int iZ1;
    unsigned int iA1;
    Libnucnet__Net__screening_function pfFunc;
    void *pUserData;
  } extra_data;
...
)
{
  ...
  /*==========================================================================
  // Allocate second data structure.
  //========================================================================*/

  p_extra_data2 = ( extra_data2 * ) malloc( sizeof( extra_data2 ) );

  if( !p_extra_data2 )
  {
    LIBNUCNET__ERROR( "Couldn't allocate memory for data structure" );
  }
  ...
  p_extra_data2->pNet = Libnucnet__Zone__getNet( p_extra_data->pZone );
  p_extra_data2->dCorr = 0.;
  p_extra_data2->dT9 = p_extra_data->dT9;
  p_extra_data2->dRho = p_extra_data->dRho;
  p_extra_data2->dYe = p_extra_data->dYe;
  p_extra_data2->pfFunc = p_extra_data->pZone->pfNseCorrectionFactorFunction;
  p_extra_data2->pUserData = p_extra_data->pZone->pNseCorrectionFactorData;
  p_extra_data2->pNseCorrFactors = p_extra_data->pNseCorrFactors;
  ...
  p_rates->dCorrection = exp( p_extra_data2->dCorr );
...
)
{
  ...
  if( !sx_species )
  {
    LIBNUCNET__ERROR( "Invalid species" );
  }
...
)
{
  ...
  if( !p_extra_data->pNseCorrFactors )
  {
    p_extra_data->dCorr -=
      p_extra_data->pfFunc(
        Libnucnet__Nuc__getSpeciesByName(
